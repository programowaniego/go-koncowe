...
int alfaBeta(plansza* stol,int glebokosc,int alfa,int beta,int* debesciak)
{
    ...
    if(!glebokosc || !glowa)
        ...
        do 
        {
            plansza* kopia = utworz_plansze(kopia,stol->gracz_na_ruchu,1,stol->wartosci); //do kopii daję wartosci ze stolu zeby miala takie same
            //lista_posuniec = dostepne_ruchy(kopia,lista_posuniec,glowa->dostepne.pozycja,1);
            //printf("%d",glowa->dostepne.pozycja);
